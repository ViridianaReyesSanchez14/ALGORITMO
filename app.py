...
class BusquedaTabu:
    def __init__(self, coord, tiempo_persistencia=3, temperatura_inicial=100, temperatura_minima=0.01, velocidad_enfriamiento=0.99, ciudad_origen=None, ciudad_destino=None):
        self.coord = coord
        ...
        # Asegurarse de que la ciudad de origen y destino estén fijas al principio y al final
    ...
    def ejecutar(self, iteraciones=100):
        iteraciones_restantes = iteraciones
        
        while iteraciones_restantes > 0 and self.temperatura > self.temperatura_minima:
            ...
            for vecino in vecinos:
                ...
                # Si no mejora, pero es mejor que la mejor ruta conocida
                    ...
                    # Criterio de aceptación basado en temperatura (probabilidad)
                    delta = dist_actual - dist_tmp
                    if random.random() < math.exp(delta / self.temperatura):
                        self.estado_actual = vecino
                        self.actualizar_memoria_tabu(vecino)
                        mejor_vecino = vecino
                        mejor_evaluacion = dist_tmp
            ...
            self.temperatura *= self.velocidad_enfriamiento
...
def calcular_ruta():
    ...
    temperatura_inicial = float(request.form['temperatura_inicial'])
    temperatura_minima = float(request.form['temperatura_minima'])
    velocidad_enfriamiento = float(request.form['velocidad_enfriamiento'])
